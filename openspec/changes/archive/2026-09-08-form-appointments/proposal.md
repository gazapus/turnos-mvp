...
- Ver/editar: click en una fila del modo Lista o en una card del modo Día. Los botones de la columna Acciones del listado siguen siendo stubs (no confirman, anulan ni llaman).
...
- `appointments-list`: click en una fila abre el mismo popup de detalle; la columna Acciones permanece stub.

## Impact

- `apps/web`: popup y formulario (RHF + Zod), primitivas de toast/dialog/tooltip, click en fila de Lista, click handlers de FullCalendar (`dateClick` / `eventClick`), slots de 15 min, asset de fondo en `public/images/agenda/`, cliente HTTP de turnos/pacientes.
...
- Fuera de alcance: colisiones/sobreturno, confirmar/anular/llamar con efecto, edición de paciente, WhatsApp.
