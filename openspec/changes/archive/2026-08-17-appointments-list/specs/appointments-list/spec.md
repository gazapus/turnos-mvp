...
El sistema SHALL exponer `GET /api/turnos` protegido por sesión autenticada (cualquier rol), que acepta filtros por médico, especialidad, paciente e inclusión de cancelados, y paginación por cursor. La respuesta MUST incluir únicamente los campos necesarios para el listado: id, fecha, hora, datos mínimos de paciente (nombre, apellido), médico (nombre, apellido) y especialidad (nombre), estado y tipo. Las respuestas MUST no cachearse (sin encabezados de caché HTTP que permitan servir una respuesta obsoleta).
...
- **THEN** cada turno de la respuesta expone únicamente id, fecha, hora, paciente (nombre y apellido), médico (nombre y apellido), especialidad (nombre), estado y tipo, sin exponer campos internos de la entidad
...
El sistema SHALL exponer endpoints de solo lectura protegidos por sesión autenticada para poblar los combos de filtro de la Agenda: listado de médicos (usuarios con rol Médico), listado de especialidades y listado de pacientes.
...
#### Scenario: Listado de pacientes para el filtro

- **WHEN** un usuario autenticado solicita el listado de pacientes
- **THEN** el sistema devuelve todos los pacientes con su id, nombre y apellido
